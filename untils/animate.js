...
export const draw_init = (ctx_temp, canvas_width_temp, canvas_height_temp, text,userconfig_msg) => {
	console.log("ok1")
	tree_root_radius1 = userconfig_msg.root_radius
	load_other= userconfig_msg.load_other/10
	interval=userconfig_msg.interval/10
	root_much=userconfig_msg.root_much
	//canvas=canvas_temp
	ctx = ctx_temp
	...
	document.fonts.load('24px fontmaobi').then(() => {
		...
		animate_tree_grow(root_much, startX_temp, startY_temp, tree_root_radius1, 2, -Math.PI / 2,
...
// 开始动画
let load_other=0//主干加载其他枝丫时的生长百分比
let interval=0//枝丫间的间隙
let root_much=0//枝干数量
function animate_tree_grow(n, startX, startY, lineWidth, length, th, tree_root_radius, isroot, rootmainway) {
	...
	//长细枝
	if (n > 0 && tree_grow_process > load_other) {
		console.log(load_other)
		//枝丫生长间隙(random大于的数值越大间隙越大)
		let random = generateRandomNumber(0, interval)
		if (random > Math.floor(interval/2)) {
			//确定生长方向(一半几率)
			let random2 = generateRandomNumber(0, 1)
			if (random2 < 0.5) {
				animate_tree_grow(Math.floor(generateRandomNumber(0, root_much)), startX, startY, lineWidth * 0.8, 2, th1 -
	...
	if (lineWidth > 0.1) {
		// 继续下一帧动画
		let Animation_index_temp=requestAnimationFrame(() => {
		...
		Animation_index.push(Animation_index_temp)
	} else {
		return
	}
...
function draw_petal(startX, startY) {
	ctx.beginPath();
	ctx.arc(startX, startY, Math.floor(generateRandomNumber(1, 4)), 0, Math.PI * 2);
	ctx.closePath();
	let color_arr = ["#ff1014", "#e34345", "#d90611"]
